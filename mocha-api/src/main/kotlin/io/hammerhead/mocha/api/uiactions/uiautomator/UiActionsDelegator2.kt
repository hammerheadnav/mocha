...
@DslTagMarker
class UiActionsDelegator2 {
    val uiDevice = UiDevice.getInstance(InstrumentationRegistry.getInstrumentation())

    internal fun clickDelegate(init: UiAction2.() -> Unit) = delegate(init, { uiDevice.findObject(it).click() })
//    internal fun clickAndWaitDelegate(delay: Long, init: UiAction2.() -> Unit) = delegate(init, { uiDevice.findObject(it).clickAndWaitForNewWindow(delay) })
...
//    internal fun takeScreenshotDelegate(file: File, init: UiAction.() -> Unit) = delegate(init, {uiDevice.takeScreenshot(file) })
    internal fun swipeUpDelegate(init: UiAction2.() -> Unit) = delegate(init, {uiDevice.findObject(it).swipe(Direction.UP,0.3f)})





    private fun delegate(init: UiAction2.() -> Unit, action: (BySelector) -> Unit) {
        UiAction2(init, action).invoke()
    }
}
