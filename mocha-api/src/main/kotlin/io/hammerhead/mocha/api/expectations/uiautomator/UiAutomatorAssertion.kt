...
class UiAutomatorAssertion(init: UiAutomatorAssertion.() -> Unit, val uiObject: UiObject) {
    var text: String by SideEffectDelegator {
        check(uiObject.text == it, { "Expected $it Actual ${uiObject.text}" })
    }
    ...
    var childCount: Int by SideEffectDelegator {
        check(uiObject.childCount == it,
                { "Expected child count $it Actual child count ${uiObject.text}" })
    }
