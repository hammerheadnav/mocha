...
class UiAutomatorAssertion(init: UiAutomatorAssertion.() -> Unit, private val uiObject: UiObject) {
    ...
    var exists: Boolean by SideEffectDelegator { check(uiObject.exists() == exists) }
    var waitForExists: Long by SideEffectDelegator { check(uiObject.waitForExists(it) == exists)  }
